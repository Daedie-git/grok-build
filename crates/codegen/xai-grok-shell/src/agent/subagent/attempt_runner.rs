            ...
            input
                ...
                .map(|method| crate::upload::manifest::ArtifactUploadContext {
                    gcs_config: crate::session::repo_changes::TraceExportConfig {
                        ...
                        prefix_dir: None,
                        gcs_prefix: Some(format!("{}/turn_0", input.child_session_id)),
            ...
            match *turn_result {
                ...
                Ok(Err(error)) => {
                    ...
                    SubagentResult {
                        ...
                        // Preserve an answer committed before a terminal
                        // failure instead of replacing partial findings with
                        // an empty result.
                        output: Arc::from(final_text),
                        ..base_result(&input, tool_calls, turns, duration_ms)
                    }
                }
                Err(_) => {
...
fn base_result(
    ...
    SubagentResult {
        ...
        child_session_id: input.child_session_id.to_owned(),
        tool_calls,
        turns,
        duration_ms,
        worktree_path: input
