...
async fn cancelled_wait_after_completion_keeps_emitted_wake_eligible() {
    ...
    let mut wait = Box::pin(adapter.wait_for_completion("pending", Some(Duration::from_secs(30))));
        ...
        tokio::select! {
            ...
            message = gateway_rx.recv() => {
                match message.expect("wait request") {
                    AcpClientMessage::WaitForTerminalExit(args) => break args.response_tx,
...
async fn cancelling_one_of_two_waiters_keeps_only_live_wait_visible() {
    ...
    assert_eq!(tasks.lock().unwrap()["pending"].active_block_waiters, 2);
